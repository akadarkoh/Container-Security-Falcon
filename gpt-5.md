Here’s a clean way to use CodePipeline to react to new images in ECR and “retrieve” them (pull, inspect, scan, or deploy). Adjust the deploy/scanning parts to your goal.

**Architecture**
- ECR → CodePipeline Source: New image/tag in an ECR repo triggers the pipeline.
- CodeBuild Stage: Logs into ECR, pulls the image, inspects/exports metadata, runs tests/scans.
- Optional Deploy: Update ECS service (or EKS) with the new image.
...
- version: 0.2
  phases:
    pre_build:
      commands:
        - echo "Reading image details from source artifact"
        - IMAGE_URI=$(jq -r '.ImageURI' imageDetail.json)
        - REPO=$(jq -r '.RepositoryName' imageDetail.json)
        - TAG=$(jq -r '.ImageTag' imageDetail.json)
    ...
    build:
      commands:
        ...
        - echo "Docker inspect"
        - docker inspect "$IMAGE_URI" | tee docker_inspect.json
        - echo "ECR manifest"
...
- Create ECR repo, push a test image, enable scan.
- Create CodeBuild project with Docker privileged mode; attach the IAM role.
- In CodePipeline:
...
Would you like this templated as CloudFormation or CDK, and are you aiming to deploy to ECS/EKS or just collect metadata/scans from ECR images?
